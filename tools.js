...
export function addText() {
    disableDrawingMode();
    const text = new fabric.IText('Double click to edit', { left: 100, top: 100, fontSize: 24, fontFamily: 'Arial' });
...
export function enableHighlightMode() { enableDrawMode(); elements.fabricCanvas.freeDrawingBrush.color = 'rgba(255, 255, 0, 0.4)'; }
export function enableWhiteoutMode() { enableDrawMode(); elements.fabricCanvas.freeDrawingBrush.color = '#ffffff'; }
...
export async function performOCR() {
    ...
    data.words.forEach(w => {
        ...
        elements.fabricCanvas.add(new fabric.IText(w.text, { left: w.bbox.x0, top: w.bbox.y0, fontSize: (w.bbox.y1-w.bbox.y0)*0.8 }));
    });
